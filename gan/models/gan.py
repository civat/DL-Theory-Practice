...
class GAN(BaseModel):
    ...
    def __init__(self, configs):
        ...
        if device == "cuda":
            self.G = torch.nn.DataParallel(self.G, device_ids=device_ids)
            self.G = self.G.to(device_id)
            self.D = torch.nn.DataParallel(self.D, device_ids=device_ids)
            self.D = self.D.to(device_id)

        # Define the loss function
        self.gan_loss = GANLoss(configs["Train"]["GAN_loss"])
    ...
    def compute_info(self, data_loader):
        ...
        with torch.no_grad():
            for data in data_loader:
                vecs_rand = data["vec"].to(self.device)            
    ...
    def setup(self, configs, log):
        ...
        macs, params = get_model_complexity_info(self.G,
                                                 input_shape,
                                                 as_strings=True,
